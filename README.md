...
Local-only baby monitor using a pinned HTTP+WebSocket control channel and WebRTC for media. Android/Linux focus for the first milestone (audio + sound detection only). See `SPEC.md` for the full product and protocol specification.
...
- Noise events delivered over a HTTP+WebSocket control channel.
...
- Native layers for audio capture and WebRTC shims in platform dirs (`android/`, `linux/`).
- Control channel uses HTTP+WebSocket with pinned cert fingerprints, nonce+signature proof-of-possession, and `/health` probe. Media is WebRTC over UDP. JSON framing is length-prefixed; canonical JSON (RFC 8785) for signed/HMAC’d payloads.
